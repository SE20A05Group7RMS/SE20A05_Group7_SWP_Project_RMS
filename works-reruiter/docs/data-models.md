...
**Schema location:** `packages/database/prisma/schema.prisma` (420 lines)

---

## Entity Overview

20 Prisma models + pgvector columns (managed via raw SQL outside Prisma).
...
| Model | Table | Purpose |
|-------|-------|---------|
| `User` | `users` | All system users (all 4 roles) |
| `Organization` | `organizations` | Enterprise organization entity |
| `OrganizationMember` | `organization_members` | User ↔ Organization join with role |

### Recruiting Domain

| Model | Table | Purpose |
|-------|-------|---------|
| `Role` | `roles` | Job role / position to fill |
